...
\begin{enumerate}[label=(\alph*)]
    ...
    \begin{equation*}
        \mathbb{P}\left(S^{c}\cap F^{c}\cap G^{c}\right).
    \end{equation*}
    By De Morgan's law we have
    \begin{equation*}
         \mathbb{P}\left(S^{c}\cap F^{c}\cap G^{c}\right) = \mathbb{P}\left((S\cup F\cup G)^{c}\right) = 1 - \mathbb{P}\left(S\cup F\cup G\right).
    \end{equation*}
    By probability addition theorem we have
    \begin{align*}
        \mathbb{P}\left(S\cup F\cup G\right) &= \mathbb{P}(S) +  \mathbb{P}(F) +  \mathbb{P}(G) -
        \\
        & - \mathbb{P}(S\cap F) - \mathbb{P}(S\cap G) - \mathbb{P}(F\cap G) + \mathbb{P}(S\cap F\cap G).
    \end{align*}
    Using the conditions of the problem, we calculate the probabilities:
    \begin{align*}
        \mathbb{P}\left(S\cup F\cup G\right) &= \dfrac{28}{100} +  \dfrac{26}{100} +  \dfrac{16}{100} -
        \\
    ...
    Hence, 
    \begin{equation*}
        \mathbb{P}\left(S^{c}\cap F^{c}\cap G^{c}\right) = 1 - \mathbb{P}\left(S\cup F\cup G\right) = \dfrac{50}{100} = \dfrac{1}{2} = 0.5.
    \end{equation*}
    
    \item The condition of (b) implies that we need to find
    \begin{equation*}
        \mathbb{P}\left(\text{\enquote{exactly one}}\right). 
    \end{equation*}
    ...
    Hence,
    \begin{equation*}
        \mathbb{P}\left(\text{\enquote{exactly one}}\right) = \dfrac{32}{100} = 0.32.
    \end{equation*}
    ...
    So we need to find
    \begin{align*}
        \mathbb{P}(E_{1}\cup E_{2}) &= \mathbb{P}(E_{1}) + \mathbb{P}(E_{1}) -\mathbb{P}(E_{1}\cap E_{2}) = 
        \\
...
\begin{enumerate}[label=(\alph*)]
    ...
    So we need to find
    \begin{equation*}
        \mathbb{P}\left(\text{\enquote{husbands earn less}}\right) = \dfrac{HL}{H} = \dfrac{248}{500} = 0.496.
    \end{equation*}
    ...
    So we need to find
    \begin{equation*}
        \mathbb{P}\left(\text{\enquote{wives and husbands earn more}}\right) = \dfrac{HM\cap WM}{HM} = \dfrac{54}{252} = 0.2143.
    \end{equation*}
    ...
    So we need to find
    \begin{equation*}
        \mathbb{P}\left(\text{\enquote{wives earn more, husbands earn less}}\right) = 
...
\begin{equation*}
    \mathbb{P}(C\mid M) = 0.05,\quad \mathbb{P}(C\mid W) = 0.25.
\end{equation*}
\begin{enumerate}[label=(\roman*)]
    \item We assume that
    \begin{equation*}
        \mathbb{P}(M) = \mathbb{P}(W).
    \end{equation*}
    ...
    By Bayes rule we have
    \begin{align*}
        \mathbb{P}(M\mid C) &= \dfrac{\mathbb{P}(C\mid M)\cdot\mathbb{P}(M)}{\mathbb{P}(C\mid M)\cdot\mathbb{P}(M) + \mathbb{P}(C\mid W)\cdot\mathbb{P}(W)} =
        \\
    ...
    \item We assume that
    \begin{equation*}
        \mathbb{P}(M) = 2\cdot\mathbb{P}(W).
    \end{equation*}
    Men and women are the whole population:
    \begin{equation*}
        \mathbb{P}(M) = 2\cdot\mathbb{P}(W) \Longrightarrow\mathbb{P}(M) = \dfrac{2}{3},\quad\mathbb{P}(F) = \dfrac{1}{3}.
    \end{equation*}
    Having considered Bayes rule we have
    \begin{equation*}
        \mathbb{P}(M\mid C) = \dfrac{0.05\cdot\frac{2}{3}}{0.05\cdot\frac{2}{3} + 0.0025\cdot\frac{1}{3}} = \dfrac{40}{41} = 0.9756\ldots
    \end{equation*}
\end{enumerate}
\end{solution}
...
Given probabilities are
\begin{equation*}
    \mathbb{P}(U_{i}) = p,\quad \mathbb{P}(D_{i}) = 1 - p.
\end{equation*}
\begin{enumerate}[label=(\alph*)]
    \item We need to find 
    \begin{equation*}
        \mathbb{P}(U_{1}D_{2}\cup D_{1}U_{2}).
    \end{equation*}
    Since the events are mutually exclusive, we have
    \begin{align*}
        \mathbb{P}(U_{1}D_{2}\cup D_{1}U_{2}) &= \mathbb{P}(U_{1}D_{2}) +\mathbb{P}(D_{1}U_{2}) = 
        \\
        & = \mathbb{P}(U_{1})\mathbb{P}(D_{2}) +\mathbb{P}(D_{1})\mathbb{P}(U_{2}) = 
        \\
        &= p(1 - p) + (1 - p)p
        \\
        &= 2p(1 - p),
    ...
    \item We need to find
    \begin{equation*}
        \mathbb{P}(U_{1}D_{2}U_{3}\cup D_{1}U_{2}U_{3}\cup U_{1}U_{2}D_{3}).
    \end{equation*}
    Since the events are mutually exclusive, we have
    \begin{align*}
        \mathbb{P}(U_{1}D_{2}U_{3}\cup D_{1}U_{2}U_{3}\cup U_{1}U_{2}D_{3}) &=  \mathbb{P}(U_{1}D_{2}U_{3}) +\mathbb{P}(D_{1}U_{2}U_{3}) +\mathbb{P}(U_{1}U_{2}D_{3}) =
        \\
        &= \mathbb{P}(U_{1})\mathbb{P}(D_{2})\mathbb{P}(U_{3}) + 
        \\
        &+ \mathbb{P}(D_{1})\mathbb{P}(U_{2})\mathbb{P}(U_{3}) + 
        \\
        &+ \mathbb{P}(U_{1})\mathbb{P}(U_{2})\mathbb{P}(D_{3}) =
        \\
...
\begin{enumerate}[label=(\alph*)]
    ...
    \begin{equation*}
         \mathbb{P}\left(\text{Smith $= (Bl, Br)$ $\cup$ Smith $= (Br, Bl)$}\right).
    \end{equation*}
    Since the events are mutually exclusive and equally probable, we have
    \begin{multline*}
        \mathbb{P}\left(\text{Smith $= (Bl, Br)$ $\cup$ Smith $= (Br, Bl)$}\right) =
        \\
        = \mathbb{P}\left(\text{Smith $= (Bl, Br)$}\right) + \mathbb{P}\left(\text{Smith $= (Br, Bl)$}\right) = \dfrac{1}{3} + \dfrac{1}{3} = \dfrac{2}{3}.
    \end{multline*}
\end{enumerate}
\end{solution}
